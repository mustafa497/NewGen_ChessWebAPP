...
(function () {
  ...
  /* ---- Google Form route ----------------------------------------
     An alternative to the Apps Script that needs no deployment. Create a
     Google Form with one short-answer question per field below, link its
     responses to a spreadsheet, then take a pre-filled link and copy the
     form id and the entry.NNN numbers out of it.

     The form id is the part between /e/ and /viewform.                 */
  var GFORM  = '';
  var GFIELD = {
    parent: '', student: '', age: '', level: '', format: '',
    phone: '', email: '', city: '', day: '', time: '', notes: '', consent: ''
  };
  ...
  var MODE = ENDPOINT ? 'server'
           : (GFORM && GFIELD.parent) ? 'gform'
           : WHATSAPP ? 'whatsapp' : 'preview';
  var live = MODE === 'server' || MODE === 'gform';
  ...
  function send() {
    ...
    if (MODE === 'gform') {
      ...
      var map = {
        parent: data.parent, student: data.student, age: data.age,
        level: data.levelText, format: data.formatText,
        phone: data.dial + ' ' + data.phone, email: data.email, city: data.city,
        day: data.day === 'No preference' ? '' : data.day,
        time: data.time === 'No preference' ? '' : data.time,
        notes: data.notes, consent: data.consent
      };
      Object.keys(map).forEach(function (k) {
        if (GFIELD[k]) body.append(GFIELD[k], map[k] == null ? '' : map[k]);
      });

      /* Google Forms sends no CORS headers, so the reply cannot be read. The
         request either leaves the browser or it does not; that is the most
         this route can tell us, and the wording below does not overclaim. */
      fetch('https://docs.google.com/forms/d/e/' + GFORM + '/formResponse', {
        ...
        busy(false);
        fail.hidden = false;
        fail.innerHTML = '<b>That did not go through.</b> The request could not leave your ' +
          'browser &mdash; check your connection. Nothing you typed has been lost, ' +
          'press <b>Request My Free Demo</b> to try again.';
      });
      return;
    }
