...
/* The spreadsheet, by the id in its URL. Opening it by id means this works
   whether the script was made from the sheet (Extensions -> Apps Script) or
   as a standalone project at script.google.com. */
var SPREADSHEET_ID = '1GEbjh4zsW4RzNj9ugdn_1EDhe8-coo8OMI3nFwK8uRg';
...
function doPost(e) {
  try {
    ...
    /* register.html posts here too; it goes to its own tabs */
    if (d.form === 'register') return register(d);
...
var STUDENT_HEADERS = [
  'Received', 'Student', 'Age', 'School / grade', 'Experience', 'Learning',
  'Parent', 'Email', 'Phone', 'City / area', 'Notes', 'Consent', 'Status'
];
var MEMBER_HEADERS = [
  'Received', 'Name', 'Email', 'Phone', 'Experience', 'Rating', 'City',
  'Interests', '18+', 'Consent', 'Status'
];

function register(d) {
  var LEVELS  = { 'new': 'New to chess', 'basics': 'Know the basics', 'regular': 'Plays regularly' };
  var FORMATS = { 'online': 'Online', 'home': 'At home', 'academy': 'At the academy' };
  var EMAIL   = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;

  var email = clean(d.email, 120);
  if (!email || !EMAIL.test(email)) return reply(422, 'A valid email address is required.');
  if (String(d.consent || '') !== 'yes') return reply(422, 'Consent to be contacted is required.');

  var level = LEVELS[String(d.level || '')] || '';
  if (!level) return reply(422, 'Please choose a chess experience level.');

  var dial = clean(d.dial, 6);
  if (dial && !/^\+\d{1,4}$/.test(dial)) dial = '+92';
  var phone  = clean(d.phone, 20);
  var digits = phone.replace(/[^0-9]/g, '');

  var sheetName, headers, row;

  if (d.type === 'student') {
    var student = clean(d.student, 80);
    var parent  = clean(d.parent, 80);
    var ageRaw  = String(d.age || '').trim();
    var age     = parseInt(ageRaw, 10);
    var format  = FORMATS[String(d.format || '')] || '';
    if (!student) return reply(422, 'A student name is required.');
    if (!ageRaw || isNaN(age) || age < 3 || age > 19) return reply(422, 'Age must be between 3 and 19.');
    if (!parent) return reply(422, 'A parent or guardian name is required.');
    if (!digits || digits.length < 7 || digits.length > 15) return reply(422, 'A valid phone number is required.');
    if (!format) return reply(422, 'Please choose how the student would like to learn.');

    sheetName = STUDENT_SHEET; headers = STUDENT_HEADERS;
    row = [
      new Date(), student, age, clean(d.school, 100), level, format,
      parent, email, "'" + (dial || '+92') + ' ' + phone,
      clean(d.city, 80), clean(d.notes, 500), 'yes', 'New'
    ];

  } else if (d.type === 'member') {
    var name = clean(d.name, 80);
    if (!name) return reply(422, 'A name is required.');
    if (digits && (digits.length < 7 || digits.length > 15)) return reply(422, 'That phone number is not valid.');
    if (String(d.adult || '') !== 'yes') return reply(422, 'Membership is for people aged 18 or over.');

    sheetName = MEMBER_SHEET; headers = MEMBER_HEADERS;
    row = [
      new Date(), name, email, phone ? "'" + (dial || '+92') + ' ' + phone : '',
      level, clean(d.rating, 40), clean(d.city, 80), clean(d.interests, 200),
      'yes', 'yes', 'New'
    ];

  } else {
    return reply(422, 'Unknown registration type.');
  }

  var lock = LockService.getScriptLock();
  lock.waitLock(20000);
  try {
    var sheet = tab(sheetName, headers);
    /* the same email on the same tab inside five minutes is a double press */
    if (recentEmail(sheet, headers.indexOf('Email'), email)) return reply(200, null, true);
    sheet.appendRow(row);
  } finally {
    lock.releaseLock();
  }
...
function recentEmail(sheet, col, email) {
  var last = sheet.getLastRow();
  if (last < 2) return false;
  var from = Math.max(2, last - 24);
  var rows = sheet.getRange(from, 1, last - from + 1, col + 1).getValues();
  var cutoff = Date.now() - 5 * 60 * 1000;
  var want = email.toLowerCase();
  for (var i = 0; i < rows.length; i++) {
    var when = rows[i][0];
    if (when instanceof Date && when.getTime() >= cutoff &&
        String(rows[i][col] || '').toLowerCase() === want) return true;
  }
  return false;
}

/** Run this once from the editor (select "setup", press Run). It asks for
    permission to use the spreadsheet and lays down all three tabs, so the
    first real submission is not also the first time anything is checked. */
function setup() {
  book();
